...
// readOnly hides upload/delete (e.g. for a future scope-limited delegation); writes are allowed by default.
...
function openUpload() {
  form.value = { title: '', category: 'referto', notes: '', takenAt: null, file: null };
...
<template>
  ...
  <div class="flex items-start justify-between gap-4">
    ...
    <Button v-if="!props.readOnly" :label="t('citizen.home.upload')" @click="openUpload">
      <template #icon>
        <Upload class="size-4" />
      </template>
    </Button>
  ...
  <Dialog
    ...
    modal
    :header="t('citizen.home.uploadDialog.title')"
  ...
  >
    <form class="space-y-4" @submit.prevent="submitUpload">
      ...
      <div class="flex justify-end gap-2">
        <Button
          type="button"
          :label="t('citizen.home.uploadDialog.cancel')"
          text
